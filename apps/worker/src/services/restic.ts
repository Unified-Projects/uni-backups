...
const RESTIC_CACHE_DIR = process.env.UNI_BACKUPS_RESTIC_CACHE_DIR || "/tmp/restic-cache";
const TEMP_DIR = process.env.UNI_BACKUPS_TEMP_DIR || "/tmp/uni-backups";
...
export function buildRepoUrl(storage: StorageConfig, repoName: string): string {
  switch (storage.type) {
    ...
    case "s3":
      // Preserve the protocol (http/https) so restic uses the correct scheme.
      // Restic defaults to HTTPS, so http:// endpoints must be kept as-is.
      const s3Endpoint = storage.endpoint ? storage.endpoint.replace(/\/$/, "") : "s3.amazonaws.com";
...
export function buildResticEnv(
  ...
  const effectiveCacheDir = storage.cache_dir || RESTIC_CACHE_DIR;
...
export async function backup(
  ...
  const result = await runRestic(args, env, 3600000, options?.onProgress); // 1 hour timeout for backups
...
export async function listSnapshots(
  ...
  const args = ["--repo", repoUrl, "snapshots", "--json"];
...
export async function prune(
  ...
  const result = await runRestic(args, env, 3600000, options?.onProgress);
